  ...
  return (
    ...
    <Table striped>
      ...
      <tbody>
        {
          copyAnswer.map((ans) => 
          <AnswerRow answer={ans} key={ans.id} voteUp={props.voteUp} setshowForm={setshowForm} setEditableAnswer={setEditableAnswer}/>)
        }
      </tbody>
    </Table>
    {/* Assuming the last Id is related to the last item on the answers */}

    {showForm ? <FormAnswer key={editableAnswer? editableAnswer.id : -1} lastId={props.answers.slice(-1)[0].id} answer={editableAnswer} 
    addAnswer={(answer) => {props.addAnswer(answer); setshowForm(false);}} 
    updateAnswer={(answer) => {props.updateAnswer(answer); setshowForm(false);}} 
    cancel={()=>{setshowForm(false); setEditableAnswer();}}></FormAnswer> 
...
function AnswerRow(props) {
  return(
    <tr><AnswerData answer={props.answer}/>
    <AnswerActions answer={props.answer} voteUp={props.voteUp} setshowForm={props.setshowForm} setEditableAnswer={props.setEditableAnswer}/></tr>
  );
}
